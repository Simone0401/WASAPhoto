...
// any error
// in questo modo controlliamo il ritorno della funzione run per vedere se ci sono stati errori
// potevamo fare varie funzioni, una per ogni funzionalità del main ma risulterebbe troppo 'complesso' per tale progetto
func main() {
	if err := run(); err != nil {
...
func run() error {
	...
	// Init logging
	// il logger server per dare informazioni in relazione al corrente stato di funzionamento del programma utili per il debugging
	logger := logrus.New()
	...
	// è una funzione che stiamo definendo in questo momento senza nome.
	// defer indica che la funzione sarà eseguita quando la funzione in cui è contentuta defer termina
	defer func() {
